...
function normalizeSection(section, manifestInfo, fallbackId) {
  ...
  const title = inferSectionTitle(section, manifestInfo, id);
...
function inferSectionTitle(section, manifestInfo, id) {
  const manifestTitle = manifestInfo.titleMap.get(id);
  const directTitle = section?.title ?? section?.metadata?.title;

  const initialTitle = pickTitleCandidate([directTitle, manifestTitle]);
  if (initialTitle) {
    return initialTitle;
  }
  ...
  for (const row of rows) {
    const rowTitle = pickTitleCandidate([
      row?.metadata?.title,
