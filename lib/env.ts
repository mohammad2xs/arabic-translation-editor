...
const ProductionEnvSchema = BaseEnvSchema.refine(
  ...
  (data) => {
    ...
    if (usingVercelBlob) {
      return data.VERCEL_BLOB_READ_WRITE_TOKEN !== undefined || data.BLOB_READ_WRITE_TOKEN !== undefined
    }
    return true
  },
  {
    message: 'VERCEL_BLOB_READ_WRITE_TOKEN or BLOB_READ_WRITE_TOKEN is required when using Vercel Blob storage',
    path: ['VERCEL_BLOB_READ_WRITE_TOKEN'],
  }
).refine(
  (data) => {
    // Check if KV storage is being used and validate configuration
    const hasKVConfig = data.KV_REST_API_URL || data.KV_REST_API_TOKEN || data.KV_URL

    if (hasKVConfig) {
      // If any KV variable is set, ensure all required ones are present
      return data.KV_REST_API_URL !== undefined && data.KV_REST_API_TOKEN !== undefined
    }
    return true
  },
  {
    message: 'KV_REST_API_URL and KV_REST_API_TOKEN are required when using KV storage',
