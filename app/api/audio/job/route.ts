...
async function processNextJob(lane: Lane) {
  ...
  try {
    ...
    const manifestKey = `audio:manifest:${job.id}`;
    const manifestJson = JSON.stringify(manifest, null, 2);

    try {
      // Save to Blob storage
      const blobUrl = await BlobStore.putText(manifestKey, manifestJson, 'application/json');
      console.log(`Audio manifest saved to Blob storage: ${blobUrl}`);

      // Update job URL to use new manifest endpoint
      job.audioUrl = `/api/audio/manifest/${job.id}`;
    } catch (blobError) {
      console.warn('Failed to save manifest to Blob storage, using filesystem:', blobError);
    ...
    // TODO: Job queue durability (replacing in-memory Maps) will be handled in Phase 4
    // TODO: SSE/polling endpoints for job status will be added in Phase 4
    // TODO: The current NDJSON streaming will be enhanced with KV-backed status in Phase 4
