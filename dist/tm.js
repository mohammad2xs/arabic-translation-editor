...
export async function tmSuggest(arabicText) {
    if (!tmCache) {
        await tmInit();
    }
...
export async function tmLearn(arabicText, englishText, metadata) {
    if (!tmCache) {
        await tmInit();
    }
    ...
    const existingIndex = tmCache.entries.findIndex(entry => normalizeArabicText(entry.arabic) === normalizedArabic);
...
export async function tmCleanup(minUsage = 1, maxAge = 30) {
    if (!tmCache) {
        await tmInit();
    }
    if (!tmCache)
    ...
    tmCache.entries = tmCache.entries.filter(entry => {
        if (entry.usageCount < minUsage)
            return false;
        if (new Date(entry.lastUsed) < cutoffDate)
            return false;
        return true;
    });
...
export async function tmExport() {
    if (!tmCache) {
        await tmInit();
    }
    return JSON.parse(JSON.stringify(tmCache));
}
export async function tmImport(data) {
    if (data.version !== '1.0') {
