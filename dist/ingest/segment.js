...
function calculateComplexity(text) {
    ...
    let complexity = 1;
    if (avgWordLength > 6)
        complexity += 1; // Long words
    if (words.length > 15)
        complexity += 1; // Long sentences
    if (text.includes('قال تعالى') || text.includes('قال رسول الله'))
        complexity += 1; // Scripture quotes
    if (/[[\]()]/.test(text))
        complexity += 1; // Contains references/citations
...
export async function segmentArabicText(text) {
