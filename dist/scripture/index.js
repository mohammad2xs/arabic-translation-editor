...
function findSurah(name) {
    ...
    return SURAHS.find(surah => surah.arabic.includes(name) ||
        surah.english.toLowerCase().includes(normalized) ||
        surah.number.toString() === normalized) || null;
}
// Normalize Quran reference
function normalizeQuranRef(match) {
...
function normalizeHadithRef(match) {
...
export async function detectScriptureRefs(text) {
    const refs = [];
...
export function getSurahInfo(number) {
    return SURAHS.find(surah => surah.number === number) || null;
}
// Validate verse range for a surah
export function isValidVerseRange(surahNumber, verse) {
