import { calculateLPR } from './complexity.js';
...
export function lpr(originalText, translatedText) {
...
export function coverage(arabicText, englishText) {
...
function calculateSemanticSimilarity(original, enhanced) {
...
function detectStructuralChanges(original, enhanced) {
    const changes = [];
...
export function drift(original, enhanced) {
...
export function assessQuality(arabicOriginal, arabicEnhanced, englishTranslation) {
    ...
    const issues = [];
    if (lprResult.issues)
        issues.push(...lprResult.issues);
    if (coverageResult.issues)
        issues.push(...coverageResult.issues);
    if (driftResult?.issues)
        issues.push(...driftResult.issues);
    let recommendation;
    if (!allPassed) {
    ...
    if (driftResult?.score !== undefined)
        scores.push(driftResult.score);
...
export function configureGuards(config) {
